...
% Save the tree reduction generator mapping separately
...
for ii = 1:len1
    ...
    for kk = 1:length(delete_list)
        trees = rmfield(gen_struct,delete_list{kk});
    end
end
...
for ii = 1:len
    ii/len
    tree_name = strcat('t',names{ii}(2:end));
    d2_gen_struct.(tree_name) = deep_unpack([],gen_struct.(names{ii}));
end
